...
<div style="padding:10px 10px 10px 10px">
	...
	<div style="padding:5px">
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitcgEditForm()">提交</a>
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="clearcgEditForm()">重置</a>
	</div>
</div>
<script type="text/javascript">
	//提交表单
	function submitcgEditForm(){
		//有效性验证
		if(!$('#cgEditForm').form('validate')){
		...
		$.post("${pageContext.request.contextPath}/Cg/updateCg.do",$("#cgEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示', data.msg);
				clearcgEditForm();
				$("#cgEditWindow").window('close');
