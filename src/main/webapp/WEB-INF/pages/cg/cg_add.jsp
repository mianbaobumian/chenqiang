...
<div style="padding:10px 10px 10px 10px">
	<form id="cgAddForm" class="cgForm" method="post">
		<table cellpadding="5" >
			<tr>
				<td>商品名:</td>
				<td colspan="10">
			...
			<tr>
				<td>采购数:</td>
				<td colspan="10">
					<input class="easyui-numberspinner" style="width:80px;" id="cgsl" name="cgsl" data-options="required:true,min:1,max:100"/>
				</td>
			</tr>
			<%--<tr>
				<td>采购单价:</td>
	...
	<div style="padding:5px">
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitcgAddForm()">提交</a>
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="clearcgAddForm()">重置</a>
	</div>
</div>
<script type="text/javascript">
	//提交表单
	function submitcgAddForm(){
		//有效性验证
		if(!$('#cgAddForm').form('validate')){
		...
		$.post("${pageContext.request.contextPath}/Cg/addCg.do",$("#cgAddForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示', data.msg);
				clearcgAddForm();
				$("#cgAddWindow").window('close');
	...
	function clearcgAddForm(){
		$('#cgAddForm').form('reset');
	}
