...
<div style="padding:10px 10px 10px 10px">
	<form id="kcEditForm" class="kcForm" method="post">
		<table cellpadding="5" >
			<tr>
				<td>商品名:</td>
				<td colspan="10">
					<input id="cg" class="easyui-combogrid" name="item_id" readonly="true"/>
			...
			<tr>
				<td>出库/入库:</td>
				<td colspan="10">
					<select id="czlx" class="easyui-combobox" name="czlx" panelHeight="auto" data-options="width:150,
			...
			<tr>
				<td>数量:</td>
				<td colspan="10">
					<input class="easyui-numberspinner" style="width:80px;" id="sl" name="sl"/>
				</td>
			</tr>
		</table>
	</form>
	<div style="padding:5px">
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitkcEditForm()">提交</a>
		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="clearkcEditForm()">重置</a>
	</div>
</div>
<script type="text/javascript">
	//提交表单
	function submitkcEditForm(){
		//有效性验证
		if(!$('#kcEditForm').form('validate')){
		...
		$.post("${pageContext.request.contextPath}/Kc/updateKc.do",$("#kcEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示', data.msg);
				clearkcEditForm();
				$("#kcEditWindow").window('close');
	...
	function clearkcEditForm(){
		$('#kcEditForm').form('reset');
	}
	...
	$(function () {
		$('#cg').combogrid({
			...
			url: "${pageContext.request.contextPath}/admin/Item/list.do",
			fitColumns: true,
			striped: true,
			editable: true,
			...
			columns: [[
				...
				{ field: 'spms', title: '商品描述', width: 200},
			]],
			keyHandler: {
